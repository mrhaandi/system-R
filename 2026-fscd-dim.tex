...
undecidable in all ranks $k$ for $k \ge 3$ (here referring to Leivant’s
...
\begin{description}
\item[Section~\ref{sec:system}:] Definition of the Parallel Intersection Type System of Bounded Dimension~(Definition~\ref{def:bounded-type-system}) and key properties such as weakening (Lemma~\ref{lem:weak}), substitution lemma (Lemma~\ref{lem:substitution-lemma}), and subject reduction (Lemma~\ref{lem:subject-reduction}).
...
{\color{blue} $R(x_1 : \bar{\sigma_1}, \ldots, x_l : \bar{\sigma_l}) := x_1 : R(\bar{\sigma_1}), \ldots, x_l : R(\bar{\sigma_l})$\\ alx: we might consider this slight change of notation 
to be coherent with traditional way of writing contexts, I can handle propagation of this style throughout the paper when we decide to go for it, and give a few examples below in blue to see what
suits us better}
\end{itemize}
\end{definition}
...
{\color{blue} \[R(a, b, c) = (\{a, b\}, \{c\}, \{a, c\})\]
alx: the blue version above is coherent with our notation, I can handle propagation of this style throughout the paper when we decide to go for it}
\end{example}
...
\begin{example}~\\
    ...
    {\color{blue} $\vdash_1 \lambda x.x\,x : (\{a, \{a\} \to b\} \to b)$}
...
{\color{blue}~\\ alx: maybe like this:\\[2ex]
...
\BinaryInfC{$ x : (\{a, \{a\} \to b\}) \vdash_1 x\,x : (b)$}
\RightLabel{\textnormal{($\Rightarrow$I)}}
\UnaryInfC{$\vdash_1 \lambda x.x\,x : (\{a, \{a\} \to b\} \to b)$}
\DisplayProof
}
...
\item $\Gamma_1 := \{x : (\{\{b, c\} \to a\})\}$ 
      {\color{blue} $x : (\{\{b, c\} \to a\})$}
\item $\Gamma_2 := \{y : (\{\{a\} \to b\}, \{\{a\} \to c\})\}$ 
      {\color{blue} $y : (\{\{a\} \to b\}, \{\{a\} \to c\})$}
\item $\Gamma_3 := \{x : (\{\{b, c\} \to a\}), y : (\{\{a\} \to b, \{a\} \to c\}), z : (\{a\})\}$
      {\color{blue} $x : (\{\{b, c\} \to a\}), y : (\{\{a\} \to b, \{a\} \to c\}), z : (\{a\})$}
\item $R_1 := \{(1,1), (2,1)\}$
\item $R_2 := \{(1,1), (1,2)\}$
\end{itemize}
...
\item $\Gamma_1 := \{x : (\{\{a, b\} \to c\})\}$ 
      {\color{blue} $x : (\{\{a, b\} \to c\})$}
\item $\Gamma_2 := \{x : (\{\{a, b\} \to c\}), y : (\{a, b, c\})\}$ 
      {\color{blue} $x : (\{\{a, b\} \to c\}), y : (\{a, b, c\})$}
\item $\Gamma_3 := \{x : (\{\{a, b\} \to c\}, \{\{a, b\} \to c\}), y : (\{a, b, c\}, \{a, b, c\})\}$ 
      {\color{blue} $x : (\{\{a, b\} \to c\}, \{\{a, b\} \to c\}), y : (\{a, b, c\}, \{a, b, c\})$}
\item $R := \{(1,1), (2,1)\}$
\end{itemize}
...
\begin{example}\label{xmp:subformula_no_blowup}
    {\color{red}Start a sentence with(?): Observe that}
...
Overall, the present notion of dimension resides between the multiset dimension (\textsf{EXPSPACE}-complete inhabitation~\cite[Theorem~34]{DudenhefnerR17}) and set dimension (undecidable inhabitation~\cite[Theorem~28]{DudenhefnerR17}), while preserving decidability of inhabitation.

\newpage

{\color{red} this does not truly belong to this section}
...
\section{Comparison with the Sequent Calculus for Intersection Logic}
...
We introduced a new syntax-directed intersection type system. It is related to classical systems, such as those of Barendregt, Coppo, and Dezani-Ciancaglini \cite{BarendregtCD83}, and of van Bakel \cite{Bakel04,Bakel11}. Unlike traditional systems, typing judgments in our system infer vectors of types rather than single types. 
%
The system uses special relations to control how information flows between different coordinates of these vectors. This makes it possible to track dependencies between components in a precise and structured way. Thanks to this vector-based organization, we can clearly control how information propagates during typing derivations. At the same time, the system remains syntax-directed and easy to follow. 
%
Finally, the use of vectors allows us to impose a natural restriction on derivations. We can require that only vectors of types of dimension at most $k$ are used, which plays a key role in the later results.

The central motivation for introducing this system is to obtain a formalism in which bounding a suitable resource leads to decidable inhabitation. In our setting, the bounded resource is the number of information-flow channels available during a derivation, which corresponds directly to the maximal dimension of the type vectors that may be used. Imposing such a bound restricts the complexity of information exchange across derivations for the same subterm, thereby curbing the expressive power just enough to recover decidability. When the maximal dimension is provided as part of the input, we show that inhabitation becomes decidable in 2-EXPTIME, while fixing this dimension as a parameter of the algorithm yields an EXPTIME decision procedure.

An earlier system \cite{DudenhefnerR17} imposes a similar dimensional restriction. We therefore provide a detailed comparison highlighting both conceptual similarities and key differences in expressiveness and technical treatment, throughout which we are also able to see that the present system offers a simpler and cleaner framework that is easier to use in practice.

At the same time, the restriction is essential: if no bound on the dimension is imposed, the inhabitation problem becomes undecidable. In fact, typability in the unrestricted system coincides with strong normalizability, explaining the sharp jump to undecidability. On the lower-bound side, we establish EXPSPACE-hardness for inhabitation when the maximal dimension is included in the input, showing that the problem is inherently complex even under bounded resources. We believe that determining the precise complexity of inhabitation in this setting would require resolving subtle and technically challenging combinatorial issues, which we leave as an open direction for future work.
