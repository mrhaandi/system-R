...
In this paper we introduce and study a system (denoted system~\textbf{R}\footnote{Not to be confused with system~\textbf{R} of de~Carvalho~\cite{Carvalho18}, in which relations play a role in the denotational semantics.}) with {\em bounded parallel intersection} providing a strict generalization of the multiset dimensional system~\cite{DudenhefnerR17}, extending (and deepening the analysis of) decidable inhabitation under dimensional bound.
...
Then these indices serve as pointers to binders in derivations, and the binders in derivations contain types.
...
The most important difference is that in $(\vdash_{\textnormal{IL}})$ reorganization of vector coordinates in premises is realized via inverse images of surjective functions (see rule (R)~\cite[Figure~1]{DudenhefnerU21}).
For example, for the function $f$ such that $f(1) = 1 = f(2)$ the inverse image of the vector $(\{a,b\})$ under $f$ is $f^{-1}((\{a,b\})) = (\{a,b\}, \{a,b\})$.
As a result, information is duplicated and there is no granular control over the \emph{relevant} part of assumptions.
Establishing such granular control can be realized via relations, generalizing functions, which is the aim of system~\textbf{R} in the present work.
...
Since the system incorporates features of both set and multiset interpretations of types, it is not immediately clear whether the typability problem falls into \textsf{NP} or \textsf{PSPACE}, as established for related systems by Rehof and Dudenhefner \cite{DudenhefnerR17typ}.
Studying this question could provide deeper insight into the algorithmic behavior of the system.
Similarly to the system \textbf{Rel} of de~Carvalho~\cite{Carvalho18}, it is interesting whether binary relations in system~\textbf{R} can be related to term evaluation by means of a Krivine machine.
...
%As the system \textbf{Rel} of de~Carvalho~\cite{Carvalho18} uses relations in its denotational models, it is interesting to see the way the binary relations in the current system~\textbf{R} fit into this kind of denotational semantics as well as how they describe computations in Krivine machine.

As the system of de~Carvalho~\cite[Definition~4.1]{Carvalho18} uses relations in its models, it is interesting to see how the binary relations in the current system~\textbf{R} fit into this kind of denotational semantics. It is also interesting to see how they describe computations in the Krivine machine.
