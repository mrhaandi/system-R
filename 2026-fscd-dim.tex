...
{\color{red} This Lemma is a weaker, move convoluted version of Lemma~\ref{lem:transform} and can be removed}
\label{lem:weak-dim}
\begin{enumerate}
...
\begin{lemma}
\label{lem:transform}
If $\Gamma \vdash_k t : \bar{A}$ and $R \subseteq \{1, \ldots, \dim(\bar{A})\} \times \{1, \ldots, m\}$ is right-unique, then $R(\Gamma) \vdash_{\max(k,m)} t : R(\bar{A})\!\!\downarrow$.
\end{lemma}
...
\begin{proof}
    ...
    By rule ($\Rightarrow$E) we obtain $R(\Gamma \cup R'(\Delta)) \vdash_n u\,v : R(\bar{A})$.\qedhere
\end{description}
\end{proof}
