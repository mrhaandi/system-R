...
\end{lemma}
\begin{proof}
    The proof is by induction over $t$. All cases follow immediately from the induction hypothesis except for the case when $t=x$ for some variable $x$. In this case the dimension restriction $n\leq k$ is replaced by $n\leq m$, which obviously holds given $k \leq m$.
\end{proof}
...
The above Lemma~\ref{lem:transform} can be used to permute, remove, or copy vector coordinates in derivations, illustrated by the following Example~\ref{xmp:deduplicate}.
...
A subtle aspect of Lemma~\ref{lem:transform} is that coordinate copies to increase the dimension besides

\begin{example}
    Let $\sigma := \{a,b,c\}$.
    We have $\{x : (\sigma), y : (\sigma \to d)\} \vdash_3 y\,x : (d)$, and $3$ is the minimal dimension necessary to derive this judgment.
    By Lemma~\ref{lem:transform}, using the relation $\{(1,1), (1,2)\}$ we have $\{x : (\sigma, \sigma), y : (\sigma \to d, \sigma \to d)\} \vdash_3 y\,x : (d, d)$.
    In particular, dimension~$3$ suffices to account for the coordinate copy shown in the following derivation:\medskip\\
...
%    such that for each judgment $\Delta\vdash_k u : B$ in this derivation
