...
\begin{itemize}
    ...
    \item $R$ is \emph{left-unique} when for each $j\in\{1, \ldots, n\}$ there is at most one $i$ such that $i\,R\,j$
    \item $R$ is \emph{right-unique} when for each $i\in\{1, \ldots, m\}$ there is at most one $j$ such that $i\,R\,j$
\end{itemize}
\end{definition}
...
\begin{proof}
    ...
    W.l.o.g. $\bar{A}$ has unique coordinates as we can remove duplicate ones by Lemma~\ref{lem:transform}.
    \begin{description}
