...
\begin{proof}
    ...
    \item[Case $t = \lambda x.v$:] In this case, reduction has actually the form 
...
An important consequence of the lemma above is that many properties of the van Bakel's system are inherited. In particular, system \textbf{R} has the strong normalization property as van Bakel's system has \cite[Theorem~7.5]{Bakel04}.
...
The main motivation for this system was to obtain a formalism in which bounding a resource, here, the number of information-flow channels corresponding to vector dimension, renders inhabitation decidable. With the maximal dimension being part of the input, inhabitation is \textsf{2-EXPTIME} decidable, and fixing it as a parameter yields an \textsf{EXPTIME} procedure. Without a bound, inhabitation becomes undecidable, as typability for such system is equivalent to strong normalizability.{\color{red} undecidability of typability has nothing to do with undecidability of inhabitation - we need to refer to Pawe\l's rank 3 paper} We also establish \textsf{EXPSPACE}-hardness when the maximal dimension is part of the input, leaving the exact complexity as an open problem.

An earlier system \cite{DudenhefnerR17} imposes a similar dimensional restriction. We therefore provided a detailed comparison highlighting both conceptual similarities and key differences in expressiveness and technical treatment, throughout which we are also able to see that system \textbf{R} offers a simpler and strictly more comprehensive framework that is easier to use in practice.
...
Another important avenue of investigation concerns type inference for system \textbf{R}. Since the system incorporates features of both set and multiset interpretations of types, it is not immediately clear whether the typability problem falls into \textsf{NP} or \textsf{PSPACE}, as established for related systems by Rehof and Dudenhefner \cite{DudenhefnerR17typ}. Studying this question could provide deeper insight into the algorithmic behavior of the system. As the system $\lambda^{\text{CIL}}$ by Wells et al.~\cite{WellsDMT02} was also developed in the context of type inference problems, it would be interesting to compare it with the current system on the technical level.
